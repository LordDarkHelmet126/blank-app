...
assert(cities.length === 38, `east polish must not add nodes, got ${cities.length}`);
const keyOf = (code) => content.regions.states.find((s) => s.short === code).keyCities.join();
assert(keyOf("NE") === "omaha", "Nebraska key is Omaha");
assert(keyOf("KS") === "topeka", "Kansas key is Topeka");
assert(keyOf("MO") === "st_louis", "Missouri key is St. Louis");
assert(cities.find((r) => r.id === "omaha").neighbors.includes("lincoln") && !cities.find((r) => r.id === "omaha").neighbors.includes("topeka"), "Omaha meets Lincoln, not Topeka");
assert(cities.find((r) => r.id === "lincoln").neighbors.includes("topeka"), "Lincoln meets Topeka");
assert(cities.find((r) => r.id === "topeka").neighbors.includes("wichita") && !cities.find((r) => r.id === "topeka").neighbors.includes("st_louis"), "Topeka meets Wichita, not St. Louis");
assert(cities.find((r) => r.id === "wichita").neighbors.includes("st_louis"), "Wichita meets St. Louis");
...
const gulf = cities.find((r) => r.id === "gulf_passage");
const cuba = cities.find((r) => r.id === "far_cuba");
const nicaragua = cities.find((r) => r.id === "far_nicaragua");
const stLouis = cities.find((r) => r.id === "st_louis");
assert(gulf && gulf.type === "sea" && gulf.unlockPhase === 3 && gulf.short === "Gulf Sealift", "Gulf Sealift is a Phase 3 sea node");
assert(stLouis.neighbors.includes("gulf_passage") && gulf.neighbors.includes("st_louis"), "St. Louis opens the Gulf Sealift");
assert(gulf.neighbors.includes("far_cuba") && cuba.neighbors.includes("gulf_passage"), "Gulf Sealift reaches Cuba");
assert(cuba.neighbors.includes("far_nicaragua") && nicaragua.neighbors.includes("far_cuba"), "Cuba still links Nicaragua");
assert(!stLouis.neighbors.includes("far_cuba") && !cuba.neighbors.includes("st_louis"), "no direct St. Louis–Cuba leap");
assert(gulf.neighbors.slice().sort().join() === "far_cuba,st_louis", "Gulf Sealift only touches St. Louis and Cuba");
...
westHop.ap = 2;
res = act(westHop, content, "travel", { regionId: "far_cuba" });
assert(!res.ok && /leap|adjacent/i.test(res.message), `no leap St. Louis→Cuba: ${res.message}`);
westHop.ap = 2;
res = act(westHop, content, "travel", { regionId: "gulf_passage" });
assert(!res.ok && /later foreign|phase|Gulf/i.test(res.message), `Gulf Sealift locked until Phase 3: ${res.message}`);
assert(!travelUnlocked(westHop, regionOf(westHop, "gulf_passage")), "Gulf phase gate before national leader");
...
assert(approachRoads(leap, "bethel", "st_louis").join(", ") === "Fairbanks, Anchorage", "St. Louis from Bethel still uses the Alaska exits");
assert(approachRoads(leap, "omaha", "topeka").join(", ") === "Lincoln", "Omaha to Topeka names Lincoln, not a leap");
assert(approachRoads(leap, "omaha", "st_louis").join(", ") === "Lincoln", "Omaha to St. Louis starts at Lincoln");
assert(approachRoads(leap, "lincoln", "st_louis").join(", ") === "Topeka", "Lincoln to St. Louis names Topeka");
assert(approachRoads(leap, "topeka", "st_louis").join(", ") === "Wichita", "Topeka to St. Louis names Wichita");
assert(approachRoads(leap, "denver", "st_louis").join(", ") === "Topeka", "Denver to St. Louis prefers the Topeka hop");
assert(approachRoads(leap, "denver", "lincoln").join(", ") === "Omaha", "Denver to Lincoln names Omaha, not Topeka");
assert(approachRoads(leap, "cheyenne", "st_louis").join(", ") === "Omaha", "Cheyenne east gate is Omaha");
assert(approachRoads(leap, "billings", "st_louis").join(", ") === "Omaha", "Billings east gate is Omaha");
assert(approachRoads(leap, "st_louis", "omaha").join(", ") === "Wichita", "West from St. Louis names Wichita");
assert(!approachRoads(leap, "denver", "st_louis").includes("Springs"), "NEXT does not send St. Louis via the Springs shortcut");
...
const eastOnly = createNewGame(content, { seed: 22, difficulty: "easy", name: "Scout", background: "scout" });
act(eastOnly, content, "raise_banner");
regionOf(eastOnly, "lincoln").owner = "northern_front";
tickCampaign(eastOnly);
assert(!eastOnly.campaign.liberated.includes("NE"), "Lincoln alone does not liberate Nebraska");
regionOf(eastOnly, "omaha").owner = "northern_front";
tickCampaign(eastOnly);
assert(eastOnly.campaign.liberated.includes("NE"), "Omaha liberates Nebraska");
regionOf(eastOnly, "wichita").owner = "northern_front";
tickCampaign(eastOnly);
assert(!eastOnly.campaign.liberated.includes("KS"), "Wichita alone does not liberate Kansas");
regionOf(eastOnly, "topeka").owner = "northern_front";
tickCampaign(eastOnly);
assert(eastOnly.campaign.liberated.includes("KS"), "Topeka liberates Kansas");
regionOf(eastOnly, "st_louis").owner = "northern_front";
tickCampaign(eastOnly);
assert(eastOnly.campaign.liberated.includes("MO"), "St. Louis liberates Missouri");
assert(!eastOnly.campaign.nationalLeader, "NE/KS/MO keys are not the west bloc");
["anchorage", "fairbanks", "juneau", "seattle", "spokane", "portland", "bend", "boise", "missoula", "cheyenne", "salt_lake"].forEach((id) => {
  regionOf(eastOnly, id).owner = "northern_front";
});
tickCampaign(eastOnly);
assert(!eastOnly.campaign.nationalLeader, "seven west states plus the east spine are not enough");
["denver", "colorado_springs"].forEach((id) => {
  regionOf(eastOnly, id).owner = "northern_front";
});
tickCampaign(eastOnly);
assert(eastOnly.campaign.nationalLeader && eastOnly.campaign.phase >= 3, "the eighth west state still opens Phase 3");
...
playerOf(lib).region = "wichita";
lib.ap = 6;
res = act(lib, content, "travel", { regionId: "st_louis" });
assert(res.ok && playerOf(lib).region === "st_louis", `Phase 3 Wichita→St. Louis: ${res.message}`);
res = act(lib, content, "travel", { regionId: "far_cuba" });
assert(!res.ok && /leap|adjacent/i.test(res.message), `still no leap St. Louis→Cuba: ${res.message}`);
res = act(lib, content, "travel", { regionId: "gulf_passage" });
assert(res.ok && playerOf(lib).region === "gulf_passage", `travel Gulf Sealift: ${res.message}`);
res = act(lib, content, "travel", { regionId: "far_cuba" });
assert(res.ok && playerOf(lib).region === "far_cuba", `travel Cuba: ${res.message}`);
...
assert(/Omaha → Lincoln → Topeka → Wichita → St. Louis/.test(uiSrc) && /East spine:/.test(uiSrc), "NEXT names the east spine");
assert(/class="states-board"/.test(uiSrc) && /NE key/.test(uiSrc), "states board and city report name NE/KS/MO keys");
const css = readFileSync(new URL("../css/game.css", import.meta.url), "utf8");
assert(/east-readability/.test(css), "readability block for coach, states, city report");
assert(/\.objective span \{ color: #f8f8f8; font-size: 13px;/.test(css), "NEXT text is 13px on white");
assert(/\.city-stats \.pill \{ font-size: 13px;/.test(css), "city report chips are 13px");
assert(/\.states-board \.pill \{ font-size: 13px;/.test(css), "states chips are 13px");
assert(/\.coach-ring[\s\S]*?animation:\s*none/.test(css), "coach ring does not blink");
